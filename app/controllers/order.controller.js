...
async function add_order(req, res) {
  try {
    ...
    const { id_payment_method, id_address, use_credits, shipping_cost, date } =
      req.body;
    ...
    if (!id_payment_method || !id_address || !shipping_cost || !date) {
      return res
        .status(400)
    ...
    if (use_credits === undefined) {
      return res
        .status(400)
        .json({ error: 'El campo use_credits es requerido' });
    }
    ...
    if (typeof use_credits !== 'boolean') {
      return res
        .status(400)
        .json({ error: 'El campo use_credits debe ser booleano' });
    }
    ...
    if (typeof shipping_cost !== 'number' || shipping_cost <= 0) {
      return res
        .status(400)
        .json({ error: 'El campo shipping_cost debe ser un número positivo' });
    }
    ...
    if (payment_method.rows.length === 0) {
      return res.status(400).json({ error: 'El método de pago no existe' });
    }
    ...
    if (use_credits) {
      ...
      total_cost =
        Number(total.rows[0].costo_total) -
        Number(credits.rows[0].creditos) +
      ...
      if (total_cost < 0) {
        ...
        credits_result = Math.floor(total_cost * 0.1);
      }
    } else {
      ...
      credits_result =
        Math.floor(total_cost * 0.1) + Number(credits.rows[0].creditos);
    }
    ...
    for (let i = 0; i < cart.rows.length; i++) {
      ...
      await pool.query(
        ...
        [
          newOrder.rows[0].id,
          cart.rows[i].id_producto,
          Product_Value.rows[0].cost_unit,
          cart.rows[i].cantidad_prod,
          cart.rows[i].observacion,
        ]
      );
    }
    ...
    await pool.query(`UPDATE datos_usuarios SET creditos = $1 WHERE id = $2`, [
      credits_result,
      id,
    ]);
...
async function orderStatus(req, res) {
  try {
    ...
    await pool.query(`UPDATE pedido SET estado = $1 WHERE id = $2`, [
      status,
      id_order,
    ]);
...
async function getDetailsRestaurant(req, res) {
  try {
    ...
    if (
      response_data.rows.length === 0 ||
      response_products.rows.length === 0
    ) {
      return res
        .status(404)
        .json({ error: 'No se encontraron pedidos para el restaurante' });
