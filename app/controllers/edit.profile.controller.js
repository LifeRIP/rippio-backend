...
async function change_password(req,res){
    try{
        ...
        if(
            ...
            return res.status(400).json({message: "la contraseña nueva debe estar igual en los dos campos"});
        }

        // Validar que la contraseña no sea igual a la actual

        if(
            oldpassword === newpassword || oldpassword === password_confirmation
          ){
            return res.status(400).json({message: "la contraseña no puede ser la misma contraseña actual"});
        }
        ...
        res.status(500).json({ message: "Ha ocurrido un error al modificar la contraseña" });
    }
...
async function add_address(req,res){
    try{
        ...
        res.status(500).json({ message: "Ha ocurrido un error al agregar la direccion" });
    }
